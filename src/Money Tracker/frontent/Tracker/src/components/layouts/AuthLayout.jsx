...
export default function AuthLayout({children}){
    ...
    return (
        ...
        <div className="w-screen h-screen md:w-[60vw] px-12 pt-8 pb-12">
            <h2 className="text-lg font-medium text-black">Money Tracker </h2>
            {children}
        </div>
        <div className="hidden md:block w-[40vw] h-screen bg-violet-50 bg-auth-bg-img bg-cover bg-no-repeat bg-center overflow-hidden p-8 relative ">
            <div className=" absolute w-48 h-48 rounded-[40px] bg-purple-600 -top-7 -left-5"></div>
            ...
            <img src="" className="w-64 lg:w-[90%] absolute bottom-10 shadow-lg shadow-blue-400/15" />
        </div>
        </div>
    )
}
...
const StatsInfoCard = ({icons,label,value,color})=>{
    return  (
        <div className="flex gap-6 bg-white p-4 rounded-xl shadow-md shadow-purple-400/10 border border-gray-200/50 z-10">
            <div className={`w-12 h-12 flex items-center justifycenter text-[26px] text-white ${color} rounded-full dro-shadow-xl `}>
                {icons}
            </div>
            <div className="">
